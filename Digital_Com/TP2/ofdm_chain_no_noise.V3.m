function [BER, SER] = ofdm_chain_no_noise(Nc, h, Npr, M, K, mPoints, mLabels, g0)
    ...
    % Convertir les données reçues du domaine fréquentiel de parallèle à série
    received_serial = parallel_to_serial(received_freq_domain);  % Convertir les données reçues en série

    % Vérifier la taille de received_serial
    expected_size = Nc * K;  % Nc sous-porteuses pour K blocs
    if numel(received_serial) ~= expected_size
        error('Le nombre d''éléments dans received_serial (%d) ne correspond pas au produit de Nc et K (%d)', ...
            numel(received_serial), expected_size);
    end

    % Appliquer l'égalisation Zero Forcing directement sur les données en série
    H_serial = repmat(H.', K, 1);  % Étendre la réponse du canal pour correspondre à toutes les données en série
    equalized_serial = received_serial ./ H_serial;  % Égalisation Zero Forcing

    % Suppression de la symétrie Hermitienne directement sur les données en série
    % Nc_half correspond à l'indice de (Ns + 1) après avoir enlevé les symétries
    Nc_half = (Nc / 2);  % Nc/2 correspond à la partie Hermitienne des données
    num_elements_per_block = Nc;  % Nombre d'éléments par bloc

    % Supprimer les éléments correspondants à la symétrie Hermitienne
    equalized_serial_no_hermitian = [];  % Initialiser un vecteur vide pour stocker les données sans Hermitien
    for block_idx = 1:K
        % Extraire un bloc du signal
        ...
        % Retirer la première sous-porteuse (DC) et les parties symétriques
        block_no_hermitian = block(2:Nc_half);  % Conserver uniquement la partie non-symétrique

        % Ajouter ce bloc traité dans le signal final sans symétrie
        equalized_serial_no_hermitian = [equalized_serial_no_hermitian; block_no_hermitian];
    end

    % Maintenant les données sont égalisées et la symétrie Hermitienne est supprimée, tout en série
